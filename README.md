...
NixpkgsとHome Managerのバージョン、local configの読み込み、Home Manager configurationを定義する。
...
```nix
{
  ...
  outputs = inputs@{ self, nixpkgs, home-manager, ... }:
    let
      ...
      # Normal operation should use local.nix with --impure.
      defaultConfig = {
        username = "new_user";
        homePrefix = "/home";
        system = "x86_64-linux";
...
また、`local.nix` が存在するのに `--impure` なしで実行された場合に、activation前に止めるガードを置く。
...
`home-manager switch -b backup` のようにflake attributeを明示しない場合、このガードに到達する前にattribute解決で失敗することがある。
